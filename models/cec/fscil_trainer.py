...
class FSCILTrainer(Trainer):
    ...
    def set_up_model(self):
        ...
        self.model = self.model.to(device) #GPU/CPU
    ...
    def replace_to_rotate(self, proto_tmp, query_tmp):
        for i in range(self.args.low_way):
            ...
            if sel_rot == 90:  # rotate 90 degree
                ...
                proto_tmp[i::self.args.low_way] = proto_tmp[i::self.args.low_way].transpose(2, 3).flip(2) #从索引i开始取low_way flip数组反转
                query_tmp[i::self.args.low_way] = query_tmp[i::self.args.low_way].transpose(2, 3).flip(2) #换二维和三维
    ...
    def get_optimizer_base(self):
        ...
        #学习率
        if self.args.schedule == 'Step': #固定步长
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=self.args.step, gamma=self.args.gamma)
        elif self.args.schedule == 'Milestone': #灵活步长
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            #【30000，32，32，3】
            train_set, trainloader, testloader = self.get_dataloader(session) #加载loader
            ...
            if session == 0:  # load base class train img label
                ...
                optimizer, scheduler = self.get_optimizer_base() #SGD 学习率调度

                for epoch in range(args.epochs_base):
                    start_time = time.time()
                    # train base sess
                    self.model.eval() #评估模式
                    tl, ta = self.base_train(self.model, trainloader, optimizer, scheduler, epoch, args) #得到的loss 和 acc
    ...
    def base_train(self, model, trainloader, optimizer, scheduler, epoch, args):
        ...
        label = torch.arange(args.episode_way + args.low_way).repeat(args.episode_query) #返回一维数组 repeat=15
        label = label.type(torch.LongTensor)

        for i, batch in enumerate(tqdm_gen, 1):
            data, true_label = [_.to(device) for _ in batch]

            k = args.episode_way * args.episode_shot #15*1
            proto, query = data[:k], data[k:]  #分support 和 query
            # sample low_way data
            proto_tmp = deepcopy(
            ...
            model.module.mode = 'encoder' #模型模块模式 编码模式
            data = model(data)
            proto_tmp = model(proto_tmp)
            query_tmp = model(query_tmp)

            # k = args.episode_way * args.episode_shot
            proto, query = data[:k], data[k:] # 15 : 225

            proto = proto.view(args.episode_shot, args.episode_way, proto.shape[-1]) # 改变维度 变成3维[1,15,64]
            query = query.view(args.episode_query, args.episode_way, query.shape[-1])# 改变维度 变成3维[15,15,64]
            ...
            proto = proto.mean(0).unsqueeze(0) #对各列求均值
            proto_tmp = proto_tmp.mean(0).unsqueeze(0) #unsqueeze 增加维度
