...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if self.args.dataset == 'cub200':
            ...
            self.num_features = 512
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1)) #自适应池化层 自适应地计算核的大小和每次移动的步长

        self.fc = nn.Linear(self.num_features, self.args.num_classes, bias=False) #线性全连接层
    ...
    def update_fc(self,dataloader,class_list,session):
        ...
        if self.args.not_data_init:
            new_fc = nn.Parameter(
                torch.rand(len(class_list), self.num_features, device="cpu"),
                requires_grad=True)
