...
def base_train(model, trainloader, optimizer, scheduler, epoch, args):
    ...
    tqdm_gen = tqdm(trainloader) #tqdm(可迭代对象)
    for i, batch in enumerate(tqdm_gen, 1): #start=1 指定索引起始值 batch=128 30000/128 = 235
        data, train_label = [_.to(device) for _ in batch]

        logits = model(data) #为什么是[128，100]
        logits = logits[:, :args.base_class] #取前60
...
def replace_base_fc(trainset, transform, model, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(trainloader):
            ...
            embedding_list.append(embedding.to(device))
            label_list.append(label.to(device))
